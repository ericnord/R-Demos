...
# in a typical R environment each line of the above output would being the the index of the first value on the line, so:
# [1] 1 2 3 ....
#[29] 29 30 31 ...
...
# Loops
for (i in 1:27) {cat(letters[i])};cat("\n") # cat("\n") just adds a new line

for (i in 1:10) {print(paste(i,"squared =",i^2))}

for (i in sample(x = 1:26,size = 14)) cat(LETTERS[i])

# The same examples w/o using for( )
paste(letters[1:27],collapse="") # first example, see what happens if paste() is not used
paste(1:10,"squared =",(1:10)^2)# second example, formatting is different
...
# a) Change the 12 to 42. b) How does this change the mean? c) How many values are greater than 40? d) What is the mean of values over 40?
...
##4)## If we  have two vectors, `a<-11:20` and `b<-c(2,4,6,8)` predict (_without running the code_) the outcome of the following (Write out your predictions as comments in your HW. After you make your predictions, you can check yourself by running the code). Were you correct? 
# a) `a*2`  
# b) `a[b]`  
# c) `b[a]`  
# d) `c(a,b)`  
